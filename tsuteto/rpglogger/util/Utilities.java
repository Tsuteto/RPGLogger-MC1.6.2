...
public class Utilities
{
    ...
    public static String getBiomeName(int biomeId, int dimension, boolean inUnderground, float py)
    {
        ...
        {
            String key = BiomeGenBase.biomeList[biomeId].biomeName;
            String fullkey = "biome." + key;
            if (msgTrans.isKeyAvailable(fullkey))
            {
                return msgTrans.getMsg("biome", msgTrans.getMsg(fullkey));
            }
            else
            {
    ...
    public static String getDimensionName(int dimId, ParamPlayer player)
    {
        RlMsgTranslate msgTrans = RlMsgTranslate.getInstance();

        String dimNameMc = player.dimensionName;
    ...
    {
        return entity instanceof EntityLiving && (entity instanceof IMob || entity instanceof EntityDragon);
    }
     ...
     * @return
     */
    public static float getTemperature(World world, int px, int pz)
    {
        try
        {
            WorldChunkManager wcm = world.getWorldChunkManager();
            return wcm.getTemperatures(null, px, pz, 1, 1)[0];
        }
        catch (Exception e)
        {
            RpgLogger.systemLog("Failed to get temperature");
            return -1;
        }
    }
    ...
    public static float getRainfall(World world, int px, int pz)
    {
        ...
        {
            RpgLogger.systemLog("Failed to get rainfall");
            return -1;
        }
    }
